...
> **Sana:** 2026-07-24
> **Bozor:** O'zbekiston (uz/ru, mobil-birinchi, Telegram-dominant, TAS-IX)
...
**Kino** — O'zbekiston bozori uchun obunaga asoslangan kino/serial **striming platformasi**, uning ajratuvchi xususiyati — har bir kinoga bog'langan **Reels** (1–3 daqiqalik vertikal teaser) tizimi.
...
|---|------|----------------------|----------|
| 1 | **Foydalanuvchi veb** | Next.js (App Router) | Browse, tomosha, reels, obuna, profil, SEO |
| 2 | **Admin veb** (alohida) | Next.js SPA + alohida auth | Kontent, foydalanuvchi, obuna, moderatsiya, analitika |
| 3 | **Mobil (Android+iOS)** | Flutter (yagona kod bazasi) | Tomosha, offline, reels, push |
| 4 | **Telegram bot + Mini App** | grammY (TypeScript) | Auth, browse, xabarnoma, to'lov deep-link |
...
| **Foydalanuvchi** | Telefon OTP orqali ro'yxatdan o'tgan, obunali/bepul |
...
1. **Modulli monolit** (mikroservis EMAS) — kichik jamoa uchun eng tez va tranzaksion izchil. Modul chegaralari toza chizilgan, keyin "issiq" modullarni (reels feed, entitlement) alohida servisga ajratish mumkin.
2. **Yagona umumiy REST API** (OpenAPI bilan hujjatlashtirilgan, `/api/v1` versiyalash) — barcha 4 yuza uchun. Admin — bir xil API'ning admin-scoped RBAC qismi.
3. **Media plane ajratilgan** — ilova server video baytlarni HECH QACHON uzatmaydi; faqat metadata + entitlement + qisqa muddatli imzolangan token beradi. Video CDN'dan to'g'ridan-to'g'ri oqadi.
4. **Entitlement — yagona haqiqat manbasi** — "bu foydalanuvchi ko'ra oladimi?" logikasi bitta joyda.

### 3.2. Arxitektura diagrammasi

```mermaid
flowchart TB
    subgraph Clients["Yuzalar"]
        W[Foydalanuvchi Veb<br/>Next.js]
        A[Admin Veb<br/>Next.js]
        M[Mobil<br/>Flutter iOS/Android]
        T[Telegram Bot + Mini App<br/>grammY]
    end

    subgraph Edge["Edge / Gateway"]
        GW[API Gateway<br/>Nginx/Traefik + WAF + Rate-limit]
    end

    subgraph Core["Modulli Monolit (NestJS yoki Go)"]
        AUTH[Identity & Auth<br/>Telefon OTP + Telegram]
        CAT[Katalog<br/>Movies/Series/Episodes]
        REEL[Reels moduli]
        ENT[Entitlement / Access]
        BILL[Obuna & To'lov<br/>Payme/Click/Uzum]
        NOTIF[Xabarnoma]
        PLAY[Playback-Token servisi]
        WORK[Async worker<br/>encoding, notif, expiry]
    end

    subgraph Media["Media Plane (ajratilgan)"]
        ING[Ingest + Transcode<br/>FFmpeg + Shaka Packager]
        STOR[(MinIO S3<br/>segmentlar/reels)]
        CDN[TAS-IX Edge CDN<br/>Nginx cache]
    end
    ...
    Core --> PG & RD & SRCH
    Core -. eventlar .-> CH
    PLAY -. imzolangan token .-> CDN
```

### 3.3. Asosiy modullar (monolit ichida)
- **Identity & Auth** — telefon OTP, Telegram bog'lash, JWT + rotatsion refresh, admin RBAC
...
- **Obuna & To'lov** — tariflar, obunalar, entitlement, promokod; Payme/Click/Uzum adapterlari + webhook
...
| **DB** | **PostgreSQL** (primary + streaming replica) | ACID (to'lov/entitlement), JSONB (moslashuvchan metadata), UZ-hosting | MySQL |
| **Kesh/sessiya/navbat** | **Redis** (+ BullMQ/Asynq) | Issiq katalog/feed keshi, OTP+refresh, rate-limit, oddiy job-queue (Kafka'siz) | — |
| **Qidiruv** | **Meilisearch** | Yengil, uz/ru typo-tolerant instant search, kam ops | Postgres FTS (MVP), OpenSearch (miqyosda) |
| **Media transcode** | **FFmpeg + Shaka Packager** (ochiq) | VOD-only → Flussonic'ning jonli TV kuchi kerak emas; per-stream litsenziya yo'q; to'liq nazorat; UZS narxida arzon | **Flussonic** — jamoada media-injener yo'q bo'lsa, tezroq ishga tushirish uchun (litsenziyali, raqobatchilar shunda) |
| **Paketlash** | HLS (CMAF/fMP4), DASH keyin | iOS/Safari native HLS; bitta fMP4 to'plam HLS+DASH; raqobatchi stack | — |
| **Kontent himoya** | Qisqa TTL imzolangan URL (HMAC), premiumga AES-128; DRM keyin | Raqobatchilar DRM'siz token ishlatadi; DRM'ning ~1% narxida casual himoya | Widevine/FairPlay (litsenziar talab qilsa) |
...
| **Veb frontend** | **Next.js 14+ (App Router, RSC)** | SEO+ISR (katalog), CSR (personal); RSC kam JS (past Android) | — |
...
| **Pleyer** | hls.js (veb), Media3/ExoPlayer (Android), AVPlayer (iOS) | Har platformada eng yaxshi bepul native pleyer, bir xil HLS chiqishi | — |
| **Analitika** | **PostHog (self-hosted, ClickHouse)** + Yandex Metrika (veb) | Ma'lumot rezidentligi (UZ qonuni), reels event hajmi, funnel/flag/A-B; Metrika — SEO/marketing | — |
...
| **SMS (OTP)** | **Eskiz.uz** (asosiy) + PlayMobile (zaxira) | UZ standart provayderlari, ikki provayder — ishonchlilik | — |
| **Orkestratsiya** | Docker + **k3s/RKE2** (stateless), DB/transcode bare-metal | Kichik jamoa uchun to'g'ri o'lchamli K8s; DB/GPU alohida temir | Nomad, docker-compose (MVP) |
| **CI/CD & IaC** | GitLab CI/GitHub Actions → Harbor → ArgoCD; Terraform + Ansible | Takrorlanuvchi, GitOps, UZ ichida registry (tez pull) | — |
| **Observability** | Grafana **LGTM** (Loki/Mimir/Prometheus/Tempo) + Sentry | Self-hosted, telemetriya egress yo'q, K8s-native | Datadog (rad — SaaS xarajat+egress) |
...
- **Title** (`id, type: movie|series, year, age_rating, poster, is_free, status, popularity`) → 1:N TitleTranslation(uz-Latn/uz-Cyrl/ru), Season, Reel, MediaAsset; N:M Genre/Person
...
- **Transaction / LedgerEntry** (double-entry, `idempotency_key`, provider, status) — pul harakati
...
- **PlaybackSession / Device** (concurrency limiti, push-token)
...
- Janr, yil, davlat, til/dublyaj, yosh-reyting, IMDb/Kinopoisk + o'z reytingi
- Freemium bayroq (`is_free`), premyera/HD/4K teglari
- Nashr holati mashinasi: `draft → transcoding → ready → published/scheduled → unpublished/archived` (nashr faqat asset tayyor bo'lganda)
...
- **ABR ladder**: 240p/360p/480p/720p/1080p H.264/AAC (past Android mos), transcode-once
...
- **Subtitr/audio**: uz/ru dublyaj audio-dorojka + WebVTT subtitr (uz-Latn/uz-Cyrl/ru), UTF-8 charset transcode ingest'da
...
- To'lov: **Payme, Click** (MVP) → **Uzum**, Uzcard/Humo (Atmos orqali binding)
- **Webhook**: imzo tekshirish + **idempotentlik** + reconciliation (majburiy — pul buzilmasligi uchun)
- Promokod/sovg'a-kod, tekshiruv muddati (trial), grace-period + dunning (avtomatik yangilanish ishonchsiz UZ'da → karta-token + eslatma)
- **Fiskal chek (OFD/soliq)** — qonun talabi, pipeline'ga o'rnatilgan
- Apple/Google IAP — alohida izolyatsiyalangan entitlement manba (server-side receipt validatsiya)

> ✅ **TASDIQLANGAN #3 — Hamyon YO'Q: to'g'ridan-to'g'ri obuna debit-at-purchase.** Saqlangan mijoz balansi UZ'da **e-money regulyatsiyasi**ni (litsenziya, rezerv, refund/dormant majburiyat) keltirib chiqaradi va daromad-tan-olishni murakkablashtiradi (8.3-bo'lim). Foydalanuvchi obunani gateway orqali to'g'ridan-to'g'ri sotib oladi. Promokod/sovg'a — obuna muddatini uzaytirish (`free_days`) sifatida, balans EMAS. *Hamyon — faqat huquqiy ko'rik + aniq mahsulot sababidan keyin (masalan TVOD/mikro-to'lov).*
...
- PWA (o'rnatiladigan, offline shell, web-push)
- Pleyer: sifat/audio/subtitr tanlash, resume, keyingi epizod avtoplay, skip-intro, PiP

### 6.7. Admin panel (alohida ilova)
- Alohida frontend + alohida auth realm (email+parol + **majburiy TOTP 2FA**, IP allowlist)
...
- HLS token-auth pleyer (Media3/AVPlayer), ABR, dublyaj/subtitr, cross-device resume
...
- **Metadata boyitish**: TMDB (asosiy) + Kinopoisk (ikkilamchi) + admin editorial override
- **KPI**: bepul→pullik konversiya, DAU/MAU, watch-time, **reel→kino CTR**, D1/D7/D30 retention, churn, MRR/ARPU
...
| **i18n** | **uz-Latn, uz-Cyrl, ru** (8.2-bo'lim — bitta "uz" EMAS) |
| **Accessibility** | WCAG 2.1 AA (veb/PWA), platforma a11y API (mobil), subtitr kafolatlangan |
| **Ma'lumot rezidentligi** | UZ fuqarolari PII UZ ichida (Personal Data Law ZRU-547) |
...
Infra xarajat **richag**larini sifat jihatdan muhokama qiladi, lekin **unit-ekonomika modeli yo'q**: to'liq-yuklangan abonent-boshiga xarajat, stream/reels-so'rov xarajati, SMS-OTP xarajati, amortizatsiyalangan litsenziya+dublyaj har-sarlavha, va 15–60k UZS narxida **break-even abonent soni**. **Yechim**: litsenziya+dublyaj+SMS+infra+gateway+store komissiya ↔ ARPU modeli; break-even; qattiq siyosat cheklovlari (SMS/user/kun); freemium bepul-sarlavha tanlovini litsenziya narxiga tekshirish.
...
- **Ma'lumot qonuni**: UZ ichida PII, column-encryption, HMAC-searchable telefon hash, consent registry, DSAR (retention-exception bilan)
